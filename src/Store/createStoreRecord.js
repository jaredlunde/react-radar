...
export default function createStoreRecord ({
  store,
  state,
  recordType,
  reducer,
  ...
  // const context = reduceProps(props, ['store', 'state', 'recordType', 'reducer'])
  ...
  if (!record) {
    ...
    // tracks all the record/store associations globally
    RegisteredRecords.register(store, key)
    // sets the state of the record
    record.setState(stateReducer)
    // invalidates other stores that are listening to the record at this key
    RegisteredRecords.invalidateStoresByKey(key, [store])
  } else {
    RegisteredRecords.register(store, key)
    record.setState(stateReducer)
  }
