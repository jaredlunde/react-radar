...
export const defaultReducer = (
  ...
  if (isSubset(state, nextState)) return state
  return deepMerge(state, nextState, {arrayMerge: arrayMergeReplace})
}

export default ({fields = emptyObj, initialState = emptyObj, reducer = defaultReducer}) => {
  // finds the key field
  ...
  if (__DEV__) {
    ...
    invariant(Object.keys(fields).length, `[${name}] must include a 'fields' option.`)
    invariant(keyField !== void 0, `[${name}] must include a Radar.Key field.`)
  }
  ...
  function Record (requestedFields, ...values) {
    ...
    if (!(__DEV__)) requiresFields = getFields(fields, requestedFields)
    ...
    return {
      ...
      fields,
      requiresFields,
      toString: () => requestedFields.length ? requestedFields : stringify(fields)
    }
  }
